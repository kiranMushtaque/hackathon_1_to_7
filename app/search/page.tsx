...
'use client';  // Make sure the file runs only on the client-side
...
const SearchPage = () => {
  ...
  if (!isClient) {
    return null; // Nothing renders until it's client-side
  }

  // Using URLSearchParams to get query parameters
  ...
  // Filtering the products based on the search parameters
  const filteredProducts = staticProducts.filter((product) => {
    ...
    const matchesTag = !tag || product.tags.some((t) => t.toLowerCase() === tag.toLowerCase());
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-gray-500 text-sm mb-4">Tags: {product.tags.join(', ')}</p>
